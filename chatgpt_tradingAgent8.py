...
def plot_market(prices: pd.Series, collective_actions: List[str]):
    ...
    for idx, action in enumerate(collective_actions):
        if action == "Buy":
            plt.scatter(idx, prices.iloc[idx], marker='^', color='green')
        elif action == "Sell":
            plt.scatter(idx, prices.iloc[idx], marker='v', color='red')
    plt.xlabel("Days")
    plt.ylabel("Price")
...
def plot_action_bars(agents: List[DualWeightAgent]):
    ...
    fig, axs = plt.subplots(1, len(agents), figsize=(18, 4), sharey=True)
