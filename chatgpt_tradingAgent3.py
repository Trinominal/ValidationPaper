...
class DualWeightAgent:
    ...
    def update_equity(self, action: Option, price: float):
        # Simple simulation: Buy = all-in, Sell = liquidate, Hold = do nothing
        if action == "Buy" and self.position == 0:
            ...
            self.position = 0
        # If holding stock, update equity value with price change
        if self.position > 0:
            self.equity = self.position * price
...
def run_experiment(days: int, agents: List[DualWeightAgent], grounds: List[Ground]):
    ...
    for day in range(days):
        ...
        daily_record = {"day": day, "price": round(price, 2), "grounds": grounds_active}
        for agent in agents:
            action = agent.choose_action(grounds_active)
            agent.update_equity(action, price)
            daily_record[agent.name] = {"action": action, "equity": round(agent.equity, 2)}
...
if __name__ == "__main__":
    ...
    results = run_experiment(100, agents, grounds)
