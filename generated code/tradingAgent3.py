...
class Agent:
    ...
    def get_option_weights(self, grounds):
        """Calculate weights for all options given current grounds"""
        option_weights = {"Buy": 0, "Sell": 0, "Hold": 0}
        ...
        for option in ["Buy", "Sell", "Hold"]:
            for g, active in grounds.items():
                ...
                weight = base * np.random.uniform(0.8, 1.2)

                # Enhanced context adjustments
                # RSI-based decisions
                if "RSI_very_oversold" in g and option == "Buy":
                    weight *= 2.0 if self.style == "aggressive" else 1.5
                elif "RSI_oversold" in g and option == "Buy":
                    weight *= 1.5 if self.style == "aggressive" else 1.2
                elif "RSI_oversold_mild" in g and option == "Buy":
                    weight *= 1.2 if self.style == "aggressive" else 1.0
                elif "RSI_very_overbought" in g and option == "Sell":
                    weight *= 2.0 if self.style == "conservative" else 1.5
                elif "RSI_overbought" in g and option == "Sell":
                    weight *= 1.5 if self.style == "conservative" else 1.2
                elif "RSI_overbought_mild" in g and option == "Sell":
                    weight *= 1.2 if self.style == "conservative" else 1.0
                
                # Moving average trends
                elif g == "MA_strong_uptrend" and option == "Buy":
                    weight *= 1.8 if self.style == "aggressive" else 1.3
                elif g == "MA_strong_downtrend" and option == "Sell":
                    weight *= 1.8 if self.style == "conservative" else 1.3
                elif g == "MA_crossover" and option == "Buy":
                    weight *= 1.3
                
                # MACD signals
                elif g == "MACD_bullish" and option == "Buy":
                    weight *= 1.4
                elif g == "MACD_bearish" and option == "Sell":
                    weight *= 1.4
                elif g == "MACD_momentum_up" and option == "Buy":
                    weight *= 1.2
                elif g == "MACD_momentum_down" and option == "Sell":
                    weight *= 1.2
                
                # Bollinger Band signals
                elif g == "BB_breakout_down" and option == "Buy":
                    weight *= 1.5 if self.style == "aggressive" else 0.8
                elif g == "BB_breakout_up" and option == "Sell":
                    weight *= 1.5 if self.style == "conservative" else 0.8
                elif g == "BB_breakout_up" and option == "Buy":
                    weight *= 1.3 if self.style == "aggressive" else 1.0
                elif g == "BB_breakout_down" and option == "Sell":
                    weight *= 1.3 if self.style == "conservative" else 1.0
                
                # Momentum signals
                elif g == "Price_strong_momentum_up" and option == "Buy":
                    weight *= 1.6 if self.style == "aggressive" else 1.2
                elif g == "Price_strong_momentum_down" and option == "Sell":
                    weight *= 1.6 if self.style == "conservative" else 1.2
                elif g == "Price_momentum_up" and option == "Buy":
                    weight *= 1.3 if self.style == "aggressive" else 1.1
                elif g == "Price_momentum_down" and option == "Sell":
                    weight *= 1.3 if self.style == "conservative" else 1.1
                
                # Volatility adjustments
                elif g == "High_volatility":
                    if self.style == "conservative":
                        weight *= 0.7 if option != "Hold" else 1.3
                    elif self.style == "aggressive":
                        weight *= 1.4
                elif g == "Low_volatility" and option != "Hold":
                    weight *= 0.9 if self.style == "aggressive" else 1.1
                
                # Position management
                elif g == "EquityBigGain" and option == "Sell":
                    weight *= 2.0 if self.style == "conservative" else 1.5
                elif g == "EquityLoss" and option == "Sell":
                    weight *= 1.8 if self.style == "conservative" else 1.2
                elif g == "EquityGainSell" and option == "Sell":
                    weight *= 1.5
                elif g == "EquityGainBuy" and option == "Buy":
                    weight *= 1.3 if self.style == "aggressive" else 1.0
    ...
    def compare_options(self, a, b, grounds):
        """Compare two options using weights"""
        weights = self.get_option_weights(grounds)
...
class WeightAggregationAgent(Agent):
    ...
    def decide(self, day, prices, agent_weights=None):
        if not agent_weights or len(agent_weights) != len(self.agents):
            ...
            aggregated_weights = {"Buy": 0, "Sell": 0, "Hold": 0}
            
            for agent_weight in agent_weights:
                for option in ["Buy", "Sell", "Hold"]:
                    aggregated_weights[option] += agent_weight[option]
